  ...
  return (
    <header className="grid h-dvh grid-cols-1 grid-rows-[auto_auto_1fr] bg-cobra bg-cover bg-[35%] bg-no-repeat md:max-h-[1133px] xl:max-h-[1024px]">
      <div className="mx-5 flex justify-center py-8 pt-7 xl:hidden">
        <img className="md:h-35 h-25 mt-3" src={Logo} alt="Logo" />
      </div>
      <nav className="mb-28 mt-10 hidden w-max justify-self-center xl:block">
        <ul className="flex flex-row items-end space-x-24 text-lg font-medium">
          <li>About us</li>
          <li>Retreats</li>
          <li>Free Classes</li>
      ...
      <img
        ...
        alt="hamburger menu"
      />
      <div className="flex flex-col justify-center gap-3 text-center font-futura xl:mt-0">
        <h1 className="text-3xl font-bold md:text-4xl xl:text-6xl">
          EMPOWERING VINYASA YOGA
        </h1>
        <h2 className="my-4 text-xl font-bold md:text-3xl xl:text-4xl">
          Online studio & Retreats
        </h2>
        <h3 className="pb-7 text-lg	font-medium	md:text-2xl xl:text-3xl">
          TO INSPIRE + UPLIFT YOU
        </h3>
        <h3 className="text-lg font-medium md:text-2xl xl:text-3xl">
          Since 2010
        </h3>
      </div>
      <GeneralButton
        text="Log in"
        customStyle="self-end justify-self-center	mb-14 md:absolute md:right-5 md:mb-0 md:top-9 xl:top-20 xl:right-36 "
      />
    </header>
  );
};
