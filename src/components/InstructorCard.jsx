...
//   }
    ...
    //   </h4>
    //   <div className={`flex sm:gap-1 md:gap-1 w-full h-full overflow-x-scroll whitespace-nowrap scroll smooth lg:overflow-x-hidden`} >
    //     {/* <Slider {...settings}> */}
    //         {insData.map((insItem)=>(
    //         <div key={insItem.id} className={`shadow-opacity-75 flex-col rounded-[10px] bg-purple shadow-xl shadow-pink hover:cursor-pointer hover:bg-pink hover:text-black 
    ...
    //         lg:h-[434px] lg:w-[395px]  lg:gap-y-[25px] relative h-full inline-block text-wrap
    //         lg:px-[40px] lg:pb-[35px] lg:pt-[25px] text-${isClick} carousel-slide `} onClick={() => clickCard(insItem.id)} >
    ...
    //   </div>
      ...
      <button onClick={nextSlide} className="absolute top-1/2 right-0 transform -translate-y-1/2">Next</button> ${index === activeCard ? 'active' : ''}
      */}
//         </div>
//     )
// }